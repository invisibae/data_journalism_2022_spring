...
```{r}
international_migration <- read_csv("data/tabula-md-statistical-handbook.csv", skip=1)
```
...
**A1**: **The suburban Washington region accounted for the largest % of international migration by region and MOntgomery County accounted for the largest % by county**

```{r}
international_migration$total_migration <- international_migration$"7/1/2019...11"

international_migration$total_migration


international_migration %>%
  filter(`State/Jurisdiction` != "Maryland") %>%
  filter(str_detect(`State/Jurisdiction`, "Region")) %>%
  mutate(total_migration_sum_all = sum(total_migration)) %>%
  group_by(`State/Jurisdiction`) %>%
  summarize(total_migration_pct = sum(total_migration) / total_migration_sum_all) %>%
  arrange(desc(total_migration_pct))
...
**A2** : **The DC metro region is accounts for about 2/3 of the state's international migration, about double the next largest region**
...
**A3** : **The Southern Maryland region experienced the largest percentage change for immigration between July 2017 and 2018 after their rate of international migration dropped by more than 50%**

```{r}
international_migration$july_17 <- international_migration$`7/1/2017`
international_migration$july_18 <- international_migration$`7/1/2018`
...
international_migration %>%
  ...
  mutate(total_migration_sum_all = sum(total_migration)) %>%
  group_by(`State/Jurisdiction`) %>%
  summarize(migration_change = (july_18 - july_17) / july_17) %>%
...
**A4** : **policies that made it harder for people to immigrate combined with a relatively good economy**
...
```{r}
domestic_migration <- read_csv("data/tabula-md-statistical-handbook-domestic.csv", skip = 1)
```
...
**A5**: **The Suburban Washington Area**

```{r}
domestic_migration$july_11 <- domestic_migration$`7/1/2011`
domestic_migration$july_12 <- domestic_migration$`7/1/2012`
domestic_migration$july_13 <- domestic_migration$`7/1/2013`
domestic_migration$july_14 <- domestic_migration$`7/1/2014`
domestic_migration$july_15 <- domestic_migration$`7/1/2015`
domestic_migration$july_16 <- domestic_migration$`7/1/2016`
domestic_migration$july_17 <- domestic_migration$`7/1/2017`
domestic_migration$july_18 <- domestic_migration$`7/1/2018`
domestic_migration$july_19 <- domestic_migration$`7/1/2019`
...
domestic_migration %>%
  ...
  filter(str_detect(`State/Jurisdiction`, "Region")) %>%
  group_by(`State/Jurisdiction`) %>%
  summarize(migration_net_all = (july_11 + 
                                   july_12 +
                                   july_13 +
                                   july_14 +
                                   july_15 +
...
**A6** **Southern Maryland, Upper Eastern Shore, and Lower Eastern Shore**
...
domestic_migration %>%
  ...
  filter(str_detect(`State/Jurisdiction`, "Region")) %>%
  group_by(`State/Jurisdiction`) %>%
  summarize(migration_net = (july_17 + july_18 + july_19)) %>%
  arrange(migration_net)
...
**A7** **The inverse of the international migration results**
