...
wv_april <- wv_ppp %>%
  ...
  summarise(loans_approved = n()) 

wv_april %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y = loans_approved)) +
  geom_point(aes(x=date_approved, y = loans_approved)) +
  geom_text(aes(x=date_approved, y = loans_approved), label = wv_april$loans_approved, hjust = .3,  vjust = -0.2) +
  scale_x_date(breaks = "1 day", minor_breaks = "1 week", labels=date_format("%m-%d-%y")) +
  labs(title = "PPP Loans: West Virginia, April 2020",
       x = "Date",
       y = "Number of PPP Loans Approved") +
...
loans_per_capita <- state_ppp_all %>%
  left_join(test, by = c("state" = "NAME")) %>%
  mutate(loans_per_100k = (total_ppp_loans / estimate) * 100000) %>%
  select(state, loans_per_100k, estimate, total_ppp_loans) %>%
  arrange(desc(loans_per_100k)) %>%
  head(10)

loans_per_capita %>%
  ggplot() + 
  geom_bar(aes(x = reorder(state, loans_per_100k), weight = loans_per_100k, fill = loans_per_100k)) +
  labs(title = "I'M RICH",
       subtitle = "States with the Most PPP Loans Per 100k",
       x = "State",
       y = "Approved Loans Per 100k Residents") +
  coord_flip() +
  scale_y_continuous(labels = comma) 
```
...
**A3.**: In general there is a positive relationship between loan amount and loan forgiveness amount. 
...
wv_under_25k$forgiveness_amount <- wv_under_25k$forgiveness_amount %>%
  replace(is.na(.), 0) 

wv_under_25k <- wv_under_25k %>%
  mutate(loan_cat = case_when(amount <= 1000 ~ "$1000 or less",
                              amount > 1000 & amount <= 5000 ~ "$1k-$5k",
                              amount > 5000 & amount <= 10000 ~ "$5k-$10k",
                              amount > 10000 & amount <= 20000 ~ "$10k - $20k",
                              amount > 20000 ~ "greater than $20k",)) %>%
  filter(forgiveness_amount < amount & amount != forgiveness_amount) %>%
...
wv_under_25k %>%
  ggplot() +
  geom_point(aes(x = amount, y = forgiveness_amount, color = loan_cat)) +
