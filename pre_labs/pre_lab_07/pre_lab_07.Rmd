...
**Answer**: **a list of naics sector codes and their corresponding industries**
...
**Answer**: **A list that seperates out the main elements of the html page (<head> and <body>) into two different lists containing their contents**
...
**Answer**: **a list of 2 that seperates the main html sections of the page (head and body) into different list elements**
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**: **there are multiple html tables on the page all with different ID's and classes**
...
**Answer**: **unfortunately it scrapes *every* html table on the page.  So we get a table of tables.**
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**: **a "dataframe of dataframes".  The second item is the dataframe of naics sector codes taht we're actually looking for**
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**: **naics sector code and description**
...
**Answer**: **voila, only the table we want**
...
**Answer** **lop off the summary column at the end of the table with the slice function**
...
**Answer** **the same two column data frame containing naics sector code and a description of that sector**
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**: **various data on the utlities sect**
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer**: **another list of two containing each of the main html page elements**
...
**Answer**: **The html code for the tabe on the page**
...
**Answer**: **Kind of a messy table that mostly maintains the table formatting from the website while nonetheless giving us the data we need**
...
**Answer**: **Here, we simplyify to a two column, one row dataframe that only provides info that we need**
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**: **The same kind of workforce statistics page but for mining, quarrying, and oil and gas extraction**
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**: **workforce stats for the construction industry**
...
**Answer**: **function "print()" prints each string as we run each line of code**
```{r}
...
**Answer**: **character vector with a list of every sector separated by commas**
```{r}
...
**Answer**: **Names each individual item in the vector and prints them**
```{r}
...
#scrape url into html
  
#find a specific table 
  
# extract one row (the second row) from that table 
...
**Answer**: **function assigns a name to each sector list item ("sector") prints each item**
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**: **same thing.  It doesn't matter what we call "___ in list_of_sectors"**
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**: **we error out before we never define 'sector name' within the function**
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer**: **assigns the numbers in this array a name ('number') and prints each number**
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer**: **naics sector code and a brief description of the sector**
...
**Answer**: **number of rows in the 'naics_industry' dataframe**
...
**Answer**: first line creates an array that spans 1-20 and assigns the numbers in that array a name ('row_number').  The second line prints each 'row number'
...
**Answer**: **employment in an industry as of November 2022**
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer**: **a sector code and description**
```{r}
# Define url of page we want to scrape
...
**Answer**: **we've added an additional column to display the url containing employment data for each industry in our 'naics_industry' talbe**
...
**Answer**: **builds the xpath for each sector based on naics**
...
**Answer**: **outputs each row each sector as its own seperate df object**
...
**Answer**: **spits out a url and xpath for each item in 'naics_industry'**
```{r}
...
**Answer**: **19 beautiful dataframes.  We have successfully scraped the employment statistics table from each version of the page**
...
**Answer**: **simplifies our dataframes and cleans up their names.  We keep only Employment (all employees)**
...
**Answer**: **returns relevant variables like URL, xpath, and description to our df**
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer**: **a little bit of column name cleanup and converting the employment number from November to a numeric value so that we are able to transform and analyze it**
...
**Answer**: **starts by creating an empty tibble, then runs our completed loop and puts our combined dataframes into the empty tibble we created at the beginning**
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**: **employment info on the mining sector**
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**: **missing overall employment.  Only gives unemployment, and as a % at that**
...
**Answer**: **Agriculture remains on the table, but its total employment is chagned to an NA value so as not to mess up any conclusions we might draw from the data**
