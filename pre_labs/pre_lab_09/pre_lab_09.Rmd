...
**Answer**: **this step groups the loans by county, summarise creates a count of total loans by county, arrange sorts by largest number of loans, and head displays only the top 10**
...
**Answer**: **creates a blank ggplot layer**
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**: **a bar graph of top counties' total number of ppp loans**
...
**Answer**: **the reorder function orders our bar graph by total_loans, making the graph a little visually pleasing and easy on the eye**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**: **coord flip function flips our x and y axes so we can read the names of the counties more easily**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**: **a little bit more padding between the county names, making them even easier to read**
...
**Answer**: **we have applied THE BEST theme**
...
**Answer**: **we used the prefab economist theme from the ggthemes packages to replicate the look of a chart you might see in The Economist**
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**: **We've added a title, x and y axis labels, a caption, and some formatting for our x-axis variable**
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**: **takes our unwieldy dataframe of maryland ppp loans, groups them by date approved, and provides a count of number of loans approved by date**
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**: **this line adds a line geometry layer over our ggplot to show trends in loans approved over time**

```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_histogram(aes(x=date_approved, weight=total_loans)) 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**: **adding the ggplot layer, adding a line layer to show the number of loans approved over time, and relabeling our date variable on the x-axis so that a reader could understand more easily**
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**: **adding a plot layer, then following that up with a geom line layer to visualize trends in numbers of loans being approved over time, then using "scale_x_date" to make our date formatting easy to understand for the reader, and finally turning the text on our x-axis to a 45 degree angle**
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
