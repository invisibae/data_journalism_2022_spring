...
**Answer**: **a relatively strong relationship between % of Trump voters in a state and % of businesses receiving PPP loans in that state**
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer**: **a weak relationship between percentage of trump voters and amount received per employee in PPP loans**
```{r}
...
**Answer**: **businesses_receiving_ppe_pct has a stronger relationship with "pct_trump" than "ppe_amount_per_employee"
```{r}
reveal_data %>%
...
**Answer**: **a tiny p value indicating a statistically significant relationship between our two variables**
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**: **a pretty big p-value indicating that the relationship between these two variables is not statistically significant**
```{r}
...
**Answer**: **Businesses in states carried by Trump had a slightly higher mean PPP loan amount but a significantly higher percentage of businesses receiving PPP loans than states carried by Clinton**
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**: **The difference isn't very significant.  Since our p-value is greater than our chosen significance level, we fail to reject the null hypothesis (English: we cannot say definitively thath Trump states received a greater amount per employees than Clinton states)**
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer**: **Exact opposite here.  Since we have such a low p-value, must reject the null hypothesis and fail to reject the alternative hypothesis (English: It's almost definitiely true that a higher percentage of businesses in Trump states received loans than in Clinton states )**
```{r}
